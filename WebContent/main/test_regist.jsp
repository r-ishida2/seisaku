	...
	<div class="content-container">
		...
		<form action="TestRegist.action" method="post">
			...
			<select name="f3">
				<option value="">--------</option>
				<option value="Python"
					<c:if test="${subject == 'Python'}">selected</c:if>>Python</option>
				<option value="Java"
					<c:if test="${subject == 'Java'}">selected</c:if>>Java</option>
				<option value="Script"
					<c:if test="${subject == 'Script'}">selected</c:if>>Script</option>
				<option value="Flask"
					<c:if test="${subject == 'Flask'}">selected</c:if>>Flask</option>
				<option value="AWS"
					<c:if test="${subject == 'AWS'}">selected</c:if>>AWS</option>
			</select>
			<label>回数</label>
			<select name="f4">
				<option value="">--------</option>
				<option value="1"
					<c:if test="${num == '1'}">selected</c:if>>1</option>
				<option value="2"
					<c:if test="${num == '2'}">selected</c:if>>2</option>
			</select>
		...
		<c:choose>
			<c:when test="${tests.size() > 0}">
				...
				<tr>
					...
					<td>${item.no}</td>
					<td>${item.name}</td>
					<td><input type="text" name="score" value="${item.score}" required></td>
				</tr>
				</c:forEach>
				</table>
			</c:when>
